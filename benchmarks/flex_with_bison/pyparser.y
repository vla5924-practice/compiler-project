...
%token DEFINED OTHER DEF CLASS INTEGERLITERAL FLOATLITERAL INTEGER FLOAT STRING IDENTIFIER Indentation EOE ARROW COLON QUOTE1 QUOTE2 BOOL FALSE INT STR IF ELSE ELIF RANGE WHILE FOR BREAK IMPORT CONTINUE DEFENITION RETURN OR AND NOT IN TRUE NONE MOD DOT RECTRIGHTBRACE COMMA ASSIGN ADD SUB MULT DIV EQUAL NOTEQUAL LESS GREATER LESSEQUAL GREATEREQUAL LEFTBRACE RIGHTBRACE RECTLEFTBRACE
%start input
...
inheritance: /* empty */
           ...
           | LEFTBRACE class_args_list RIGHTBRACE
            {
                $$ = $2;
            }
;
...
/* FUNCTION */
func_def: DEF funcname LEFTBRACE func_args_list RIGHTBRACE COLON suite
        {
            string fnc_name = $2;
            int indent = @1.last_column;
...
;
star_arg: MULT IDENTIFIER
            {
                $$ = $1 + $2;
            }
        | MULT MULT IDENTIFIER
            {
                $$ = $1 + $2 + $3;
            }
;
...
;
func_call_params: LEFTBRACE RIGHTBRACE
                    {
                        call_params = "";
                        $$ = $1 + $2;
                    }
                | LEFTBRACE call_params RIGHTBRACE
                    {
                        call_params = $2;
                        $$ = $1 + $2 + $3;
                    }
