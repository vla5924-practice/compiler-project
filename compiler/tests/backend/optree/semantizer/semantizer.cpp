...
class SemantizerTest : public ::testing::Test {
    Operation::Ptr dummy;
  ...
  public:
    SemantizerTest() : dummy(Operation::make("UnboundOp")), m(), v(m.values()){};
...
TEST_F(SemantizerTest, succeeds_on_valid_arith_binary_op) {
    ...
    assertNoErrors(m.nestedOp());
}

TEST_F(SemantizerTest, succeeds_on_arith_binary_op_with_incorrect_num_operands) {
    m.op<ArithBinaryOp>(makeValue(m.tI64)).attr(ArithBinOpKind::AddI).result(m.tI64);
    assertAnyErrors(m.nestedOp());
}
