...
TEST(Node, can_construct_with_fp_literal_value) {
    float value = 1.0f;
    Node node(value);
...
TEST(Node, can_construct_with_variable_name) {
...
TEST(Node, can_construct_with_type_id) {
    TypeId type_id = 1;
    Node node(type_id);
...
TEST(Node, can_construct_with_binary_operation) {
...
TEST(Node, can_construct_with_unary_operation) {
